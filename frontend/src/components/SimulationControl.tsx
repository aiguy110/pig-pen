...
export const SimulationControl: React.FC<SimulationControlProps> = ({
  ...
  const handleStart = async () => {
    ...
    if (numGames > 10000) {
      setError("Cannot start simulation with more than 10,000 games");
      return;
    }
  ...
  const handleNumGamesChange = (value: number) => {
    ...
    if (value > 10000) {
      setLimitWarning(
        "Higher values are temporarily disallowed while I try to track down a memory leak",
      );
    } else {
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Run between 1 and 10,000 games
          </p>
          {limitWarning && (
        ...
        <button
          onClick={handleStart}
          disabled={starting || selectedBots.length < 2 || numGames > 10000}
